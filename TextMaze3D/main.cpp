...
int main(int arc, char **argv)
{
   ...
   while (command != 27) {    // Loop until ESC key (#27) is pressed
      ...
      if(command > 32 && command < 127) cout << command << endl;
      ...
      if (command == 0 || command == 224) { // special key pressed (cursor etc)
         ...
         case 72:  // Up Arrow Key (move forward)
            ...
            switch (player.dir) {
            case NORTH:
               if (player.y > 0 && maze[player.y][player.x].n) player.y--;
               break;
            case EAST:
               if ((player.x < MAZE_SIZE_X - 1) && maze[player.y][player.x].e) player.x++;
               break;
            case SOUTH:
               if ((player.y < MAZE_SIZE_Y - 1) && maze[player.y][player.x].s) player.y++;
               break;
            case WEST:
               if (player.x > 0 && maze[player.y][player.x].w) player.x--;
               break;
            }
            break;
...
void generateMaze(Maze maze[MAZE_SIZE_Y][MAZE_SIZE_X], Player &player)
{
   ...
   // We'll loop as long as there is a valid location to visit in the history stack
    do {
      ...
      vector<char> check;
      if (loc.x > 0 && maze[loc.y][loc.x - 1].visited == 0) check.push_back('W'); // Check west exit.
      if (loc.y > 0 && maze[loc.y - 1][loc.x].visited == 0) check.push_back('N'); // Check north exit.
      if (loc.x < (MAZE_SIZE_X - 1) && maze[loc.y][loc.x + 1].visited == 0) check.push_back('E'); // Check east exit.
      if (loc.y < (MAZE_SIZE_Y - 1) && maze[loc.y + 1][loc.x].visited == 0) check.push_back('S'); // Check south exit.

      if (check.size()) {   // If there were unvisited exits available
   ...
   player.x = 0;
   player.y = 0;
   if (maze[0][0].e) player.dir = EAST;
   else player.dir = SOUTH;
}
            ...
            for (int x = 0; x < MAZE_SIZE_X; x++) {
               ...
               switch (r) {
                  case 0:
                     // North
                     if (x) {
                        if (maze[y][x].visited || maze[y][x - 1].visited) N1 = '+';
                        if (y) {
                     ...
                     if (y) {
                        if (maze[y][x].visited && !maze[y][x].n) N2 = '-';
                        if (maze[y - 1][x].visited && !maze[y - 1][x].s) N2 = '-';
                     }
                     else {
                        N1 = '+';
                        N2 = '-';
                  ...
                  case 1:
                     // West
                     if (x > 0) {
                        if (!maze[y][x].w && maze[y][x].visited) cout << '|' << mark;
                        else if (!maze[y][x - 1].e && maze[y][x-1].visited) cout << '|' << mark;
                        else cout << ' ' << mark;
                     }
                     else cout << '|' << mark;
                     break;
